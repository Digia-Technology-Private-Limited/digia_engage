...
/**
 * Routes resolved campaigns onto the overlay controller's state flows. Pure
 * presentation wiring — event emission (CEP / analytics) lives in
 * [com.digia.engage.internal.event.EngageEventEmitter], owned by [DigiaInstance].
 */
internal class DisplayCoordinator(
    private val overlayController: DigiaOverlayController,
) {
    fun routeNudge(config: NudgeConfig, payload: CEPTriggerPayload, variables: Map<String, String> = emptyMap()) {
        Logger.verbose("Showing nudge: id=${payload.cepCampaignId} displayStyle=${config.surface.displayType}")
    ...
    fun routeInlineCarousel(config: InlineCarouselConfig, payload: CEPTriggerPayload) {
        Logger.verbose("Showing inline carousel: id=${payload.cepCampaignId} slotKey=${config.slotKey}")
    ...
    fun routeInlineStory(config: InlineStoryConfig, payload: CEPTriggerPayload) {
        Logger.verbose("Showing inline story: id=${payload.cepCampaignId} slotKey=${config.slotKey}")
