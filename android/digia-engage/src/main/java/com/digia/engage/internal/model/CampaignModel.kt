...
internal data class CampaignModel(
    ...
    companion object {
        fun fromJson(json: JSONObject): CampaignModel? {
            val id =
                    json.optString("id", "").ifBlank { json.optString("_id", "") }.takeIf {
                        it.isNotBlank()
                    }
                            ?: return null
            val campaignKey =
                    json.optString("campaignKey", "").takeIf { it.isNotBlank() } ?: return null
            val campaignType =
                    json.optString("campaignType", "").takeIf { it.isNotBlank() } ?: return null
        ...
        private fun buildGuideConfig(
            ...
            for (i in 0 until stepsArr.length()) {
                ...
                val anchorKey =
                        stepJson.optString("anchorKey", "").takeIf { it.isNotBlank() } ?: continue
                ...
                steps.add(
                        GuideStepModel(
                                ...
                                displayStyle = displayStyle
                                                ?: stepJson.optString("displayStyle", "tooltip"),
