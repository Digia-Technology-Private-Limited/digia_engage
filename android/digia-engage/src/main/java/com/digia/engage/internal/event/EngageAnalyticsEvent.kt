 ...
 *
 * Events are grouped by campaign type — [NudgeEvent], [GuideEvent], [SurveyEvent],
 * [CarouselEvent], [StoriesEvent] — so each subtype exposes *exactly* the fields
 * its matrix row defines (ISP): a nudge event cannot carry `has_welcome`, a guide
 * step cannot omit its `item_index`. Each leaf owns its wire [eventName] and how
 * its typed fields serialize — [columns] hoisted to the payload's top level,
 * [properties] nested. Field→wire-key mapping (e.g. step index → `item_index`)
 * lives in the leaf, the single source of truth (SRP).
 *
 * These are Digia-only. CEP forwarding uses the coarse
 * [com.digia.engage.DigiaExperienceEvent] on a separate channel — the two concerns
 * are deliberately not unified.
 */
internal sealed interface EngageAnalyticsEvent {
    val eventName: String
    val columns: Map<String, Any?> get() = emptyMap()
...
internal sealed interface NudgeEvent : EngageAnalyticsEvent {
    data class Viewed(
        ...
        override val eventName get() = "Digia Experience Viewed"
        override val columns get() = nonNull("display_style" to displayStyle)
        override val properties get() = nonNull("screen_name" to screenName) + trigger.orEmpty()
    }
    ...
    data class Clicked(
        ...
        override val eventName get() = "Digia Experience Clicked"
        override val columns get() = nonNull(
            "element_id" to elementId,
            "cta_label" to ctaLabel,
            "action_type" to actionType,
            "action_url" to actionUrl,
        )
        override val properties get() = nonNull(
            "cta_role" to ctaRole,
    ...
    data class Dismissed(
        ...
        override val eventName get() = "Digia Experience Dismissed"
        override val properties get() = nonNull("dwell_ms" to dwellMs)
    }
}
...
internal sealed interface GuideEvent : EngageAnalyticsEvent {
    data class Viewed(
        ...
        override val eventName get() = "Digia Experience Viewed"
        override val columns get() = nonNull("display_style" to displayStyle, "item_total" to itemTotal)
        override val properties get() = nonNull("screen_name" to screenName) + trigger.orEmpty()
    }
    ...
    data class StepViewed(
        ...
        override val eventName get() = "Digia Step Viewed"
        override val columns get() = nonNull(
    ...
    data class StepClicked(
        ...
        override val eventName get() = "Digia Step Clicked"
        override val columns get() = nonNull(
            "item_index" to itemIndex,
            "element_id" to elementId,
            "cta_label" to ctaLabel,
    ...
    data class StepDismissed(
        ...
        override val eventName get() = "Digia Step Dismissed"
        override val columns get() = nonNull("item_index" to itemIndex)
    }
    ...
    data class Completed(
        ...
        override val eventName get() = "Digia Experience Completed"
        override val columns get() = nonNull("item_total" to itemTotal)
        override val properties get() = nonNull("time_to_complete_ms" to timeToCompleteMs)
    }
}
...
internal sealed interface SurveyEvent : EngageAnalyticsEvent {
    ...
    data class Clicked(
        ...
        override val eventName get() = "Digia Experience Clicked"
        override val columns get() = nonNull("element_id" to elementId)
    }
    ...
    data class QuestionViewed(
        ...
        override val eventName get() = "Digia Question Viewed"
        override val columns get() = nonNull(
    ...
    data class QuestionAnswered(
        ...
        override val eventName get() = "Digia Question Answered"
        override val columns get() = nonNull(
            "question_id" to questionId,
            "question_type" to questionType,
    ...
    data class QuestionSkipped(
        ...
        override val eventName get() = "Digia Question Skipped"
        override val columns get() = nonNull("question_id" to questionId, "item_index" to itemIndex)
        override val properties get() = nonNull("block_type" to blockType, "block_id" to blockId)
    }
    ...
    data class Completed(
        ...
        override val properties get() = nonNull(
            ...
            "response" to response.ifEmpty { null },
        )
    }
}
...
internal sealed interface CarouselEvent : EngageAnalyticsEvent {
    data class Viewed(
        ...
        override val properties get() = nonNull("screen_name" to screenName)
    }
    ...
    data class StepViewed(
        ...
        override val eventName get() = "Digia Step Viewed"
        override val columns get() = nonNull("item_index" to itemIndex, "item_total" to itemTotal)
        override val properties get() = nonNull("item_id" to itemId, "auto" to auto)
    }
    ...
    data class StepClicked(
        ...
        override val properties get() = nonNull("item_id" to itemId)
    }
...
internal sealed interface StoriesEvent : EngageAnalyticsEvent {
    ...
    data class Opened(
        ...
        override val eventName get() = "Digia Experience Clicked"
        override val columns get() = nonNull("element_id" to "story_open")
        override val properties get() = nonNull("story_id" to storyId)
    }
    ...
    data class StepViewed(
        ...
        override val eventName get() = "Digia Step Viewed"
        override val columns get() = nonNull("item_index" to itemIndex, "item_total" to itemTotal)
        override val properties get() = nonNull("story_id" to storyId, "frame_id" to frameId)
    }
    ...
    data class StepClicked(
        ...
        override val properties get() = nonNull("frame_id" to frameId)
    }
