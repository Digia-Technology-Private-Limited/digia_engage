...
struct ShareContentProcessor {
    ...
    func execute(action: ShareContentAction, context: ActionProcessorContext) async throws {
        guard let message = ScopeValueResolver.resolveAny(action.data["message"], in: context.scopeContext) as? String,
              !message.isEmpty else {
            throw ActionExecutionError.unsupportedContext(processorType)
        }
        let subject = ScopeValueResolver.resolveAny(action.data["subject"], in: context.scopeContext) as? String
        DigiaRuntime.shared.share(message: message, subject: subject)
        #if canImport(UIKit)
