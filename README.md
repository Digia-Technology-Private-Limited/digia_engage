# digia_engage

Core Digia Engage SDK repository — server-driven in-app experiences for mobile apps.

## Packages

| Platform | Directory | Version | Registry | Docs |
|---|---|---|---|---|
| Flutter | [`flutter/`](flutter/) | 1.1.1 | [pub.dev](https://pub.dev/packages/digia_engage) | [Flutter SDK guide](https://docs.digia.tech/engagement/digia-engage-flutter) |
| iOS | [`iOS/`](iOS/) (submodule → [digia_engage_iOS](https://github.com/Digia-Technology-Private-Limited/digia_engage_iOS)) | 1.0.0 | [Swift Package Index](https://swiftpackageindex.com/Digia-Technology-Private-Limited/digia_engage_iOS) | — |
| Android | [`android/`](android/) | 1.0.0 | [Maven Central](https://central.sonatype.com/artifact/tech.digia/engage) | [Android SDK guide](https://docs.digia.tech/engagement/digia-engage-android) |
| React Native | [`react-native/`](react-native/) | [npm](https://www.npmjs.com/package/@digia-engage/core) | — |

## Structure

- `iOS/` — Native iOS SDK (git submodule: [digia_engage_iOS](https://github.com/Digia-Technology-Private-Limited/digia_engage_iOS))
- `android/` — Native Android SDK (`digia_engage`)
- `flutter/` — Flutter SDK package (`digia_engage`)
- `react-native/` — React Native bridge

## Quick Links

- [Flutter README](flutter/README.md)
- [Digia Engage docs](https://docs.digia.tech/engagement/digia-engage)
- [Flutter SDK docs](https://docs.digia.tech/engagement/digia-engage-flutter)
- [Android SDK docs](https://docs.digia.tech/engagement/digia-engage-android)
- [MoEngage integration](https://docs.digia.tech/engagement/moengage-integration)
- [CleverTap integration](https://docs.digia.tech/engagement/clevertap-integration)
- [Digia Studio](https://app.digia.tech)
- [Issues](https://github.com/Digia-Technology-Private-Limited/digia_engage/issues)
